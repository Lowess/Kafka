 ...
 *
*/
class PartitionsComparator implements Comparator {
 	
 	@Override
    public int compare(Object o1, Object o2) {
        Partition p1 = (Partition)o1;
        Partition p2 = (Partition)o2;

        avgP1 = 0
        p1.getReplicaList().each { rep ->
        	avgP1 += pa.getPartitionLeaderCount(rep.getBroker().getBrokerId(), "RTB")
        }
        avgP1 /= p1.getReplicaList().size()

        avgP2 = 0
        p2.getReplicaList().each { rep ->
        	avgP2 += pa.getPartitionLeaderCount(rep.getBroker().getBrokerId(), "RTB")
        }
        avgP2 /= p2.getReplicaList().size()

        return avgP1.compareTo(avgP2);
    }
}
